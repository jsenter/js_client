...
function SetPlayerSprite(player){
	$('#playersprite-'+player).css('background-image',"url('http://127.0.0.1/gfx/Sprites.png')");
	//$('#playersprite-'+player).css('background-color','yellow');
	...
	if(y>0){ y = '-' + y; }
	if(x>0){ x = '-' + x; }
	$('#playersprite-'+player).css('width:32px; width:32px;');
...
function AddPlayerSprite(player,x,y){
	console.log("AddPlayerSprite: x = " + x + ", y = " + y);
	AddPlayerElement('<div id="playersprite-'+player+'" class="playersprite" style="width:32px; height:32px; left:'+x+'px;top:'+y+'px"></div>');
}

function PlacePlayer(player,tile){
	ptpos = $('#map').position();
	console.log("PlacePlayer: x = " + ptpos.left + ", y = " + ptpos.top);
	AddPlayerSprite(player, (tile[0]*32), (tile[1]*32));
