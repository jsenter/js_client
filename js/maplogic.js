...
function InitialiseMapGrid(layer){
	...
	if(layer=='floor'){ cellbackgroundcolor = 'green'; }
...
function SetMapCellImageForEditor(cellnumber, pos, layer)
{
	$('#'+layer+'-'+cellnumber).css('background-image', "url('http://127.0.0.1/gfx/Tiles.png')");
	$('#'+layer+'-'+cellnumber).css('background-position', pos);
	
}

// Sets a single map cell.
function SetMapCellImage(cellnumber, x,y,layer){
	//console.log("SetMapCellImage - layer = " + layer + ", cellnumber = " + cellnumber + ", tile = " + tile);
	// Set the background image for an individual tile.
	//$('#'+layer+'-'+(cellnumber)).css('background-image', "url('http://127.0.0.1/gfx/maps/"+tile + ".png')");
	$('#'+layer+'-'+cellnumber).css('background-image', "url('http://127.0.0.1/gfx/Tiles.png')");
	
	//console.log( "SetMapCellImage - MapTiles[tileName] = " + MapTiles[tile] + ", tile=" + tile );
	//x = MapTiles[tile]*32;
	//var x = x*32;
	if(x > 0) x = '-'+x;
	if(y > 0) y = '-'+y;
...
function SetAnimatedMapCellImage(cellnumber,x){
	x = MapTiles[x]*32;
	if(x > 0){ x = '-'+x; }
	$('#mask-'+cellnumber).html('<div id="animatedtile'+cellnumber+'" style="width:32px;height:32px;background-image:url(http://127.0.0.1/gfx/maps/'+current_active_map+'.png);background-position:'+x+'px 0px"></div>');
}
...
function SetupMapGrid(data){
	...
	AddMapElement('<div id="uilayer" style="position:absolute; width:640px;height:480px;"></div>'); // TEMP UILAYER because it is not supposed to exist yet but PlaceAllPlayersOnMap  adds to it and cannot do it if it's not there.
	
	// Over Player Sprite.
	AddMapElement('<div id="spritelayer" style="position:absolute; width:640px;height:480px;"></div>');
...
function HandleMapData(data){
	...
	//console.log("HandleMapdata - mapdata[2]: " + mapdata[2]);
...
function SetTilesDown(tiledata,layer){
	try{
		//console.log("SetTilesDown - tiledata : " + tiledata);
		tiledata = tiledata.split('=');
		//tiledata[0] = parseInt(tiledata[0]);
		//tiledata[1] = parseInt(tiledata[1]);
		 
		var pos = tiledata[1].split('x');
		// tiledata[0] - tile资源所在格子编号 tiledata[1] - tile资源文件名
		var x = pos[0]*32;
		var y = pos[1]*32;
		
		SetMapCellImage(tiledata[0],x,y,layer);
	}catch(error){}
}
...
function ExportLayerToData(layer){
	...
	for(var i = 1; i<=300; i++){
		...
		if($(cellid).css('background-image') != 'none' && $(cellid).css('background-position').substring(0,2) != '0%'){
			...
			layerdata = layerdata +i+'='+celltile[0]/32+'x'+Math.floor(celltile[1]/32)+'-';
		}
	}
